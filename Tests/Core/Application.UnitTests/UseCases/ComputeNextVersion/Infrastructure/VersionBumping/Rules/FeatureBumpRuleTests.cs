...
public class FeatureBumpRuleTests
{
    private readonly FeatureBumpRule _rule;

    public FeatureBumpRuleTests()
    {
        _rule = new FeatureBumpRule();
    }

    [Fact]
    public void ShouldImplementBaseProperties()
    {
        _rule.Priority.ShouldBe(30);
        _rule.Name.ShouldBe(nameof(FeatureBumpRule));
    }
    ...
    public async Task Apply_ShouldBumpVersion(string currentRelease, string expectedNextRelease)
    {
        ...
        {
            { "feature/item-a", new DomainVersion(1, 1, "feature/item-a", currentRelease) }
        };

        var applyResult = await _rule.Apply("feature/item-a", 1, currentVersions, context: null);
        ...
        newVersion.Meta.ShouldBe("feature-item-a");
    }
    ...
    public void CanApply_ShouldBeTrueWhenIsQA()
    {
        ...
        var canApply = _rule.CanApply("feature/item-a", currentVersions, context: null);
    ...
    public void CanApply_ShouldBeFalseWhenIsNotQA()
    {
        ...
        var canApply = _rule.CanApply("not-feature/item-a", currentVersions, context: null);
